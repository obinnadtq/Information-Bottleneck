...
for i in range(len(pz_y)):
    for j in range(len(pz_y)):
        if i != j:
            p_z_combined = p_z[i] + p_z[j]
            pi = [p_z[i] / p_z_combined, p_z[i] / p_z_combined]
            p_bar = pi[0] * px_z[j] + pi[1] * px_z[i]
            DKL1 = np.sum(px_z[j] * (np.log(px_z[j] + 1e-31) - np.log(p_bar + 1e-31)))
            DKL2 = np.sum(px_z[i] * (np.log(px_z[i] + 1e-31) - np.log(p_bar + 1e-31)))
            JS_1_2 = pi[0] * DKL1 + pi[1] * DKL2
            py_z_i = pz_y[i] * p_y / p_z[i]
            py_z_j = pz_y[j] * p_y / p_z[j]
            ...
            dist = JS_1_2 - (1 / bet) * JS_3_4
            deltaLMax = p_z_combined * dist
for k in range(len(beta)):
    Nz = Ny
    pz_y = np.zeros((Nz, Ny))
    for i in range(0, Nz):
